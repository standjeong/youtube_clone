...
## 🌳프로젝트 의의
- Youtube API를 사용하는 방법을 익힐 수 있었다.
- API를 연결하여 개발하는 과정에서 정해진 일일 API 사용량를 넘어설 경우 데이터를 읽어오지 못하는 문제를 방지하기 위하여 Mock Data와 실제 API가 연결된 상태의 로직을 따로 분리하여 사용해보는 연습을 할 수 있었다.
- API로부터 응답을 받아오는 과정에서 비동기 처리를 위해 async/await 을 학습하여 요청 함수에 적용해 볼 수 있었다.
- API 요청 과정에서 에러 발생 시 HTTP 상태 코드를 확인하여 해당 에러 코드에 따라 사용자의 경험을 방해하지 않고 에러에 따른 UI 처리를 해볼 수 있었다.
    - Youtube API 사용량 제한으로 인한 403 에러 반환 시에는 Mock Data를 자동으로 연결하여 읽어오도록 처리.
    - 클라이언트에서 무한스크롤을 제한하여 에러를 발생시키는 400 에러 반환 시에는 무한스크롤이 더이상 작동하지 않는다는 안내 문구 표시.
- 각 리소스에 맞는 API 요청 함수를 Youtube라는 클래스 내부의 메서드로 정의하여 이 클래스를 인스턴스화하여 각 리소스 요청을 쉽게 호출할 수 있도록 하였다. 이 과정에서 클래스를 통해 코드의 재사용성을 높이고 조직화하는 방식에 대해 이해할 수 있었다.
