...
**- ✅스크롤이 내려진 상태에서 목록의 동영상을 클릭하여 <VideoDetail>페이지로 들어오면 이전에 스크롤 된 상태가 유지되어서 페이지 상단이 보이지 않는 문제**
- 해결 : VideoDetail 페이지에 useEffect를 사용하여 경로가 변경될 때(=videoid가 바뀔 때)마다 페이지 상단으로 스크롤 되도록
